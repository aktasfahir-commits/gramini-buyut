...
function parseTRNumber(raw) {
  ...
  // Uzmanpara bazen ondalık noktalı format da döner (ör. 101,07)
  if (s.includes(',')) {
...
async function main() {
  ...
  try {
    ...
    silver = parseUzmanparaSilver(milliyetHtml);
  ...
  // Tamamen boş ve eski altın verisi varsa eski dosyayı koru (bayat gümüş korunmaz)
